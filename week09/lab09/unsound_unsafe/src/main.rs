...
unsafe fn list_delete_first(list: *mut List<i32>) -> *mut List<i32> {
    ...
    // /!\ SAFETY: We've checked that list is not null,
    //             so it's safe to dereference and get the next node
    let next = unsafe { (*list).next };

    // /!\ SAFETY: The list is not null, so it's a valid heap-allocated
    //             node that we can free
    unsafe {
        free_node(list);
    }
